...
echo -e "${CYAN}"
echo "#########################################################"
echo "#      Start downloading and installing  subssupport      #"
echo "#########################################################"
echo -e "${NC}"

IPK_URL="https://raw.githubusercontent.com/Ham-ahmed/110/refs/heads/main/enigma2-plugin-extensions-subssupport_1.8.0-r2-py3_all.ipk"
IPK_NAME="enigma2-plugin-extensions-subssupport_1.8.0-r2-py3_all.ipk"
IPK_PATH="/var/volatile/tmp/$IPK_NAME"

echo -e "${YELLOW}Downloading the subssupport...${NC}"
wget -O $IPK_PATH $IPK_URL

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ subssupport has been downloaded successfully....${NC}"
else
    echo -e "${RED}✗ Failed to download subssupport...${NC}"
    exit 1
fi
