...
echo -e "${CYAN}"
echo "#########################################################"
echo "#      Start downloading and installing  temperaturmonitor      #"
echo "#########################################################"
echo -e "${NC}"

IPK_URL="https://raw.githubusercontent.com/Ham-ahmed/110/refs/heads/main/enigma2-plugin-extensions-temperaturmonitor_2.0_all.ipk"
IPK_NAME="enigma2-plugin-extensions-temperaturmonitor_2.0_all.ipk"
IPK_PATH="/var/volatile/tmp/$IPK_NAME"

echo -e "${YELLOW}Downloading the temperaturmonitor...${NC}"
wget -O $IPK_PATH $IPK_URL

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ temperaturmonitor has been downloaded successfully....${NC}"
else
    echo -e "${RED}✗ Failed to download temperaturmonitor...${NC}"
    exit 1
fi
