...
echo -e "${CYAN}"
echo "#########################################################"
echo "#   Start downloading and installing  wireguard-vpn     #"
echo "#########################################################"
echo -e "${NC}"

IPK_URL="https://raw.githubusercontent.com/Ham-ahmed/110/refs/heads/main/enigma2-plugin-extensions-wireguard-vpn_15.9_all.ipk"
IPK_NAME="enigma2-plugin-extensions-wireguard-vpn_15.9_all.ipk"
IPK_PATH="/var/volatile/tmp/$IPK_NAME"

echo -e "${YELLOW}Downloading the wireguard-vpn...${NC}"
wget -O $IPK_PATH $IPK_URL

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ wireguard-vpn has been downloaded successfully....${NC}"
else
    echo -e "${RED}✗ Failed to download wireguard-vpn...${NC}"
    exit 1
fi
