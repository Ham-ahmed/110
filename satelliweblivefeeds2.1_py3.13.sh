...
echo -e "${CYAN}"
echo "###################################################################"
echo "#      Start downloading and installing  satelliweblivefeeds      #"
echo "###################################################################"
echo -e "${NC}"

IPK_URL="https://raw.githubusercontent.com/Ham-ahmed/110/refs/heads/main/enigma2-plugin-extensions-satelliweblivefeeds_2.1-r3_cortexa15hf-neon-vfpv4_py3.13.ipk"
IPK_NAME="enigma2-plugin-extensions-satelliweblivefeeds_2.1-r3_cortexa15hf-neon-vfpv4_py3.13.ipk"
IPK_PATH="/var/volatile/tmp/$IPK_NAME"

echo -e "${YELLOW}Downloading the satelliweblivefeeds...${NC}"
wget -O $IPK_PATH $IPK_URL

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ satelliweblivefeeds has been downloaded successfully....${NC}"
else
    echo -e "${RED}✗ Failed to download satelliweblivefeeds...${NC}"
    exit 1
fi
